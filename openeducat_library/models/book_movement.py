...
class OpBookMovement(models.Model):
    ...
    barcode = fields.Char('Barcode', size=20, compute='get_barcode')
    ...
    def _check_date(self):
        if self.issued_date >= self.return_date:
            raise ValidationError(
                _("Return Date should be greater than Issue Date."))
    ...
    def get_barcode(self):
        for movement_rec in self:
            movement_rec.barcode = movement_rec.book_unit_id.barcode
    ...
    def book_return(self):
        ''' function to return book from wizard'''
        ctx = dict(self._context)
        ctx.update({'default_book_id': self.book_id.id,
                    'default_book_unit_id': self.book_unit_id.id,
    ...
    @api.multi
    def book_lost(self):
        ''' function to lost book from wizard'''
        ctx = dict(self._context)
        ctx.update({'default_book_id': self.book_id.id,
                    'default_book_unit_id': self.book_unit_id.id,
                    'library_id':self.library_id.id})
